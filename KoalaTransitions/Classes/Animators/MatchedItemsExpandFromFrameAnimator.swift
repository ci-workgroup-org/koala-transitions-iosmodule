...
public protocol Framed {
    ...
    func frameIn(controller: UIViewController) -> CGRect
}

extension UIView: Framed {
    public func frameIn(controller: UIViewController) -> CGRect {
        guard isDescendant(of: controller.view) else { return CGRect.zero }
        return convert(frame, to: controller.view)
    }
}

public struct FramedAnimation {
    let toView: UIView
    let fromView: UIView
}

public class MatchedItemsExpandFromFrameAnimator: NSObject, Animator {
    ...
    public var animations = [FramedAnimation]()
